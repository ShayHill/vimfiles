...
# creates 7 new highlight groups for a total of 9 StatusLine highlight groups.
#
# * StatusLineBold (bold text for default statusline)
# * StatusLine  " previously existing
# * StatusLineWeak (grayed out text for default statusline)
# * StatusLineNCBold (bold text unfocused statusline)
# * StatusLineNC  " previously existing
# * StatusLineNCWeak (grayed out text for default statusline)
# * StatusLineCNBold (bold text for focused statusline with splits)
# * StatusLineCN  (normal text for focused statusline with splits)
# * StatusLineWeak (grayed out test for focused statusline with splits)
#
# ---------------------------------------------------------------------------- #
...
# order of preference.
if !exists("g:statusline_cursors")
    g:statusline_cursors = ["IncSearch", "Search", "ErrorMsg"]
endif
...
def HiFgOrBg(sources: list<string>, fg_or_bg: string): dict<string>
    ...
    def RecurHiFgOrBg(hlgs: list<string>): dict<string>
        var hidict = hlget(hlgs[0], v:true)[0]
...
def HiGrounds(sources: list<string>): dict<string>
    ...
    # **display** guifg.
    var source = hlget(sources[0], v:true)[0]
...
def BoldHi(base_hi_group: string, basename: string = ''): void
    ...
    #     no basename is given, a new group `base_hi_group .. 'Bold'` will be
    #     created.
    # Effects:
    ...
    hldict.name = basename == '' ? base_hi_group .. 'Bold' : basename .. 'Bold'
...
def WeakHi(base_hi_group: string, basename: string = ''): void
    ...
    #  the input highlight group
    var hldict = hlget(base_hi_group, v:true)[0]
    hldict.name = basename == '' ? base_hi_group .. 'Weak' : basename .. 'Weak'
    ...
    var gui_mixed = MixColors(grounds.guifg, grounds.guibg, 0.35)
    ...
    var cterm_mixed = MixColors(grounds.ctermfg, grounds.ctermbg, 0.35)
...
def PickCursorColor(candidates: list<string>): string
...
def SplitHi(hi_group: string, basename: string = ''): void
    ...
    # * create basename .. 'Bold' from hi_group
    # * create basename .. 'Weak' from hi_group
    #
    # If no basename is given
    # * create hi_group .. 'Bold' from hi_group
    # * create hi_group .. 'Weak' from hi_group
    if basename == ''
        BoldHi(hi_group)
        WeakHi(hi_group)
        return
    ...
    hlset([hldict])
    BoldHi(hi_group, basename)
    WeakHi(hi_group, basename)
enddef

var cursor_hi = PickCursorColor(g:statusline_cursors)
SplitHi('StatusLine')
